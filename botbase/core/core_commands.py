...
class CoreLogic:
    ...
    async def _version_info(self):
        ...
        -------
        dict
            `botbase` and `discordpy` keys containing version information for both.
        """
        return {"botbase": __version__, "discordpy": discord.__version__}
...
class Core(CoreLogic):
    ...
    async def info(self, ctx: commands.Context):
        ...
        red_repo = org_repo + "/botex-discordbot"
    ...
    async def sentry(self, ctx: commands.Context, on_or_off: bool):
        ...
        be disabled if you have made your own modifications to the botbase
        package.
        """
    ...
    async def listlocales(self, ctx: commands.Context):
        ...
        async with ctx.channel.typing():
            red_dist = pkg_resources.get_distribution("botex-discordbot")
            red_path = Path(red_dist.location) / "botbase"
    ...
    async def backup(self, ctx, backup_path: str = None):
        ...
        from botbase.core.data_manager import basic_config, instance_name
        from botbase.core.drivers.red_json import JSON

        data_dir = Path(basic_config["DATA_PATH"])
        if basic_config["STORAGE_TYPE"] == "MongoDB":
            from botbase.core.drivers.red_mongo import Mongo
