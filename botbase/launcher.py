...
from botbase.core import __version__
from botbase.core.utils import safe_delete
from botbase.core.cli import confirm
...
def update_red(dev=False, reinstall=False, voice=False, mongo=False, docs=False, test=False):
    ...
    print("Updating...")
    # If the user ran botbase-launcher.exe, updating with pip will fail
    # on windows since the file is open and pip will try to overwrite it.
    # We have to rename botbase-launcher.exe in this case.
    launcher_script = os.path.abspath(sys.argv[0])
    old_name = launcher_script + ".exe"
    new_name = launcher_script + ".old"
    renamed = False
    if "botbase-launcher" in launcher_script and IS_WINDOWS:
        renamed = True
    ...
    if test:
        egg_l.append("test")
    if dev:
        package = "git+https://github.com/BotEX-Developers/botbase@stable"
        if egg_l:
            package += "#egg=botex-discordbot[{}]".format(", ".join(egg_l))
    else:
        package = "botex-discordbot"
        if egg_l:
    ...
    if code == 0:
        print("Bot base has been updated")
    else:
        print("Something went wrong while updating!")

    # If botbase wasn't updated, we renamed our .exe file and didn't replace it
    scripts = os.listdir(os.path.dirname(launcher_script))
    if renamed and "botbase-launcher.exe" not in scripts:
...
def run_red(selected_instance, autorestart: bool = False, cliflags=None):
    while True:
        ...
        cmd_list = ["botbase", selected_instance]
        if cliflags:
            cmd_list += cliflags
...
def cli_flag_getter():
    print("Would you like to enter any cli flags to pass to botbase? (y/n)")
    resp = user_choice()
    if resp == "n":
        return None
    elif resp == "y":
...
def debug_info():
    pyver = sys.version
    redver = pkg_resources.get_distribution("botex-discordbot").version
    if IS_WINDOWS:
...
async def is_outdated():
    # red_pypi = "https://pypi.python.org/pypi/botex-discordbot"
...
def main_menu():
    if IS_WINDOWS:
        os.system("TITLE Bot Base Launcher")
    clear_screen()
    ...
    while True:
        ...
        if outdated:
            print("Bot is outdated. {} is available.".format(new_version))
        print("")
...
def main():
    if not PYTHON_OK:
        raise RuntimeError(
            "Bot base requires Python 3.6 or greater. Please install the correct version!"
        )
